...
def test_kv_is_faster_smoke():
    ...
    enc = tok("hello world", return_tensors="pt", add_special_tokens=False)
    cfg = SamplerCfg()

    t0 = time.time(); cached_generate(model, enc["input_ids"], 16, tok.eos_token_id, cfg); t1 = time.time()
    t2 = time.time(); sample_generate(model, enc["input_ids"], 16, tok.eos_token_id, cfg); t3 = time.time()
